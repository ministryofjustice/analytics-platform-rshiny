...
RUN  sed -i 's/deb/deb [trusted=yes]/g' /etc/apt/sources.list \
  ...
  && wget --quiet -O /tmp/analytics-platform-shiny-server.tar.gz https://github.com/ministryofjustice/analytics-platform-shiny-server/archive/refs/heads/fix-socket-reconnect-error.tar.gz \  && gdebi -n /tmp/r_amd64.deb \
