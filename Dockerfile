...
RUN sed -i 's,deb,deb [trusted=yes],g' /etc/apt/sources.list && \
    ...
    sed -i s,http://security.ubuntu.com/ubuntu/,https://mirror.mythic-beasts.com/ubuntu/,g /etc/apt/sources.list && \
    sed -i s,http://archive.ubuntu.com/ubuntu/,https://mirror.mythic-beasts.com/ubuntu/,g /etc/apt/sources.list && \
